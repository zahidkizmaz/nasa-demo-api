...
class NasaApiService:
    ...
    async def get_apod(self) -> dict[str, str]:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                self.APOD_URL, params=settings.NASA_API_KEY
            ) as response:
                return await response.json()


class FakeNasaApiService:
    async def get_apod(self) -> dict[str, str]:
        return {
...
def get_nasa_api_service() -> NasaApiService | FakeNasaApiService:
    if settings.name == "Prod":
        return NasaApiService()
    else:
        return FakeNasaApiService()
